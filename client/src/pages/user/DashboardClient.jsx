...
// Palette de couleurs premium Pegasio - Mise à jour avec une emphase orange, bleu, vert
const colors = {
  primary: {
    darkBlue: '#0A3D62',          // Bleu profond et riche
    vibrantOrange: '#FF7F00',     // Orange vif et engageant
    lightOrange: '#FFA740',       // Light orange for the button - NEW ADDITION/ADJUSTMENT
    emeraldGreen: '#00B894',      // Vert émeraude, frais et moderne
    skyBlue: '#74B9FF',           // Bleu ciel doux
    lightGreen: '#A8E6CF',        // Vert clair, apaisant
    white: '#F8FAFC',             // Blanc chaud, pour un fond doux
    darkRed: '#8B0000',           // Rouge sombre
  },
  secondary: {
    gray: '#6B7280',              // Gris neutre
    lightGray: '#E5E7EB',         // Gris très clair
  },
  status: {
    success: '#00B894',           // Vert succès (reprend emeraldGreen)
    warning: '#FF7F00',           // Orange attention (reprend vibrantOrange)
    error: '#EF4444',             // Rouge erreur
  }
};

// Données exemple pour les graphiques Pegasio
...
const StatCard = ({ title, value, icon, trend, duration = 0.2, delay = 0 }) => {
  ...
  let iconBgColor;
  if (iconColor === colors.primary.vibrantOrange) {
    iconBgColor = `${colors.primary.vibrantOrange}${bgColorOpacity}`;
  } else if (iconColor === colors.primary.darkBlue) {
    iconBgColor = `${colors.primary.darkBlue}${bgColorOpacity}`;
  } else if (iconColor === colors.primary.emeraldGreen) {
    iconBgColor = `${colors.primary.emeraldGreen}${bgColorOpacity}`;
  } else if (iconColor === colors.primary.skyBlue) {
    iconBgColor = `${colors.primary.skyBlue}${bgColorOpacity}`;
  } else {
    iconBgColor = `${colors.primary.darkBlue}${bgColorOpacity}`;
  }
    ...
    >
      ...
      >
        <div className="flex justify-between items-center flex-wrap gap-4">
          <div>
            <h1 className="text-4xl font-extrabold text-gray-900 leading-tight">Bonjour, {clientData.userName} 👋 </h1>
            <p className="text-lg mt-2" style={{ color: colors.primary.darkRed }}>Bienvenue dans votre espace client personnalisé Pegasio</p>
          </div>
      ...
      >
        {/* Statistiques principales Pegasio */}
        ...
        {/* Graphiques de performance Pegasio */}
      ...
      >
        ...
        {/* Carte de fidélité Premium - Button 1250/2000 REMOVED */}
        <motion.div
...
<p className="font-bold text-base" style={{ color: 'black' }}>
  Une réduction exclusive de 30 % vous attend sur notre prochain produit phare  🔥
                ...
                {/* The button was here and has been removed */}
              </div>
            </div>
          </div>
        </motion.div>
