...
const handleSendMessage = async () => {
  ...
  // 2. Ajout immédiat du message utilisateur à l'interface
  const userMsg = {
    content: newMessage,
    sender: "user",
    timestamp: new Date()
  ...
  try {
    ...
    // 5. Gestion des réponses d'erreur HTTP
    if (!response.ok) {
    ...
    // 6. Traitement de la réponse réussie
...
// Version optimisée pour la gestion de la touche Entrée
const onKeyPress = (e) => {
  ...
  <motion.span
    ...
    initial={{ opacity: 0 }}
    animate={{ 
      opacity: [0, 1, 0], // De transparent à opaque puis à nouveau transparent
      scale: [1, 1.05, 1] // Léger zoom pendant l'apparition
    }}
    transition={{
      ...
      repeatDelay: 0.5 // Temps d'arrêt entre chaque cycle
    }}
  >
    {t("hero.title")}
  </motion.span>
    ...
    <div className={`max-w-[75%] rounded-lg p-3 ${
      ...
      {msg.sender === 'ai' && (
        <div className="flex items-center mb-1">
          <span className="font-bold text-xs mr-2">PEGASIO AI</span>
        </div>
      )}
      <p>{msg.content}</p>
