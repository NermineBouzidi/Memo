...
// Nouvelle route pour les demandes de démo (sans authentification)
router.post("/demo", async (req, res) => {
  try {
    const { message, userEmail, metadata } = req.body;
    ...
    // Création du ticket avec un type spécial
    const ticket = new SupportTicket({
      userId: "demo-request",
      message: message,
      userEmail: userEmail,
    ...
    // Envoi d'email avec un template spécial pour les démos
    await sendSupportEmail({
      userEmail: userEmail,
      message: message,
      ticketId: ticket._id,
